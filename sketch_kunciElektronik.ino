...
Keypad permana = Keypad( makeKeymap(keys), PinBaris,   PinKolom, JmlBaris, JmlKolom ); 
void setup() { 
  ...
  delay(500);
  lcd.clear();
  permana.addEventListener(keypadEvent);
}
void loop() {
  pinMode(led, OUTPUT);
   if (a==0) { //state awal
  ...
  permana.getKey(); //membaca keypad
}
void keypadEvent(KeypadEvent eKey){ 
  switch (permana.getState()){
...
if (password.evaluate()){ //jika password cocok
  ...
  a = 0; 
     ...
     lcd.setCursor(0, 1); //jika password salah
     lcd.print("================");
